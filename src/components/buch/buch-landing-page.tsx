...
import { GutscheinForm } from "@/components/buch/gutschein-form";
...
/**
 * Gemeinsame Buch-Landingpage (Design der Gewinnspiel-Seite):
 * - variant "ads": Zielseite für Display-Ads (/das-buch)
 * - variant "gutschein": Zielseite des Newsletters (/gutschein) – zusätzlich
 *   Gutschein-Aktion (50 % Wiresoft-Shop) mit Formular direkt unter dem Hero.
 * Gewinnspiel erscheint in beiden Varianten als sekundärer Teaser.
 */

export type BuchLandingVariant = "ads" | "gutschein";
...
/** UTM-Parameter (z. B. aus Anzeige/Newsletter) aus den searchParams ziehen. */
...
export function BuchLandingPage({
  ...
  variant: BuchLandingVariant;
  utm: UtmParams;
  /** Nur variant "gutschein": signiertes Formular-Token. */
  formToken?: string;
  /** Nur variant "gutschein": verfügbare Codes (0 → Aktion beendet). */
  vouchersAvailable?: number;
}) {
  const env = getEnv();
  const contactIsPlaceholder = CONTACT_EMAIL.startsWith("[");
  const utmQuery = new URLSearchParams(utm as Record<string, string>).toString();
  const gewinnHref = utmQuery ? `/gewinn?${utmQuery}` : "/gewinn";
  const isGutschein = variant === "gutschein";
  ...
  return (
    <div className="gewinn-theme min-h-screen">
      ...
      <GewinnTracking
        ...
        pageEventName={isGutschein ? "gutschein_seite" : "buch_seite"}
      />
      ...
      <main id="inhalt">
        ...
        <section className="relative overflow-hidden" aria-labelledby="hero-heading">
          ...
          <div className="relative mx-auto grid max-w-6xl grid-cols-1 gap-y-9 px-5 pt-12 pb-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_380px] lg:grid-rows-[auto_auto] lg:gap-x-20 lg:gap-y-7 lg:pt-20 lg:pb-24">
            <div className="gw-fade lg:col-start-1 lg:row-start-1">
              ...
              {isGutschein ? (
                <p className="mt-4 max-w-xl rounded-xl border border-[var(--gw-gold)]/35 bg-[var(--gw-gold)]/[0.07] px-4 py-3 text-base text-[var(--gw-ink)]">
                  <strong className="font-semibold text-[var(--gw-gold-strong)]">
                    Dein Dankeschön:
                  </strong>{" "}
                  Buch bestellen, Bestellnummer eintragen – und sofort einen{" "}
                  {GUTSCHEIN_RABATT_LABEL}-Gutschein auf das gesamte Sortiment im{" "}
                  {GUTSCHEIN_SHOP_NAME} erhalten.
                </p>
              ) : null}
            </div>
            ...
            <div className="gw-fade lg:col-start-1 lg:row-start-2">
              ...
              <div className="mt-8 flex flex-col gap-4 sm:items-start">
                ...
                {isGutschein ? (
                  <a
                    href="#gutschein"
                    data-gw-event="gutschein_scroll"
                  ...
                  >
                    ...
                    Schon bestellt? Gutschein jetzt abholen
                  </a>
                ) : (
                  <a
                    href="#musikvideo"
        ...
        {/* ---------------------------------------------------------------- */}
        {/* 2 · Gutschein-Aktion (nur /gutschein)                             */}
        {/* ---------------------------------------------------------------- */}
        {isGutschein ? (
          <section
            id="gutschein"
          ...
          >
            <div className="mx-auto max-w-4xl px-5 py-16 sm:px-8 lg:py-20">
              <SectionHeading
                kicker="Exklusiv für Leser"
                title={`Dein ${GUTSCHEIN_RABATT_LABEL}-Gutschein für den ${GUTSCHEIN_SHOP_NAME}`}
                intro="Bestelle das Buch, trage deine Bestellnummer ein – und dein Gutscheincode erscheint sofort auf dem Bildschirm."
              />
                ...
                {[
                  {
                    ...
                    text: "Bei Amazon oder im Buchhandel deiner Wahl.",
                  },
                  {
                    icon: BookOpen,
                    title: "Bestellnummer eintragen",
                    text: "Du findest sie in deiner Bestellbestätigung.",
                  },
                  {
                    icon: TicketCheck,
                    title: "Code sofort erhalten",
                    text: "Er wird nicht per E-Mail versendet – direkt kopieren oder notieren.",
                  },
                ].map((step, index) => (
                  <li
                    key={step.title}
                    className="rounded-2xl border gw-hairline bg-[var(--gw-surface)] p-5"
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-b from-[var(--gw-gold-strong)] to-[var(--gw-gold-deep)] text-sm font-bold text-[#181207]">
                        {index + 1}
                      </span>
                      <step.icon className="h-5 w-5 text-[var(--gw-gold)]" aria-hidden="true" />
                    </div>
                    <p className="mt-3 font-semibold text-[var(--gw-ink)]">{step.title}</p>
                    <p className="mt-1 text-sm text-[var(--gw-ink-soft)]">{step.text}</p>
                  </li>
                ))}
              </ol>

              <div className="mt-8">
                {vouchersAvailable > 0 && formToken ? (
                  <GutscheinForm
                    formToken={formToken}
                    utm={utm}
                  ...
                  <div className="rounded-2xl border gw-hairline bg-[var(--gw-surface)] p-8 text-center sm:p-12">
                    <Ticket
                      className="mx-auto h-10 w-10 text-[var(--gw-gold)]"
                      aria-hidden="true"
                      strokeWidth={1.5}
                    />
                    <h3 className="mt-5 text-2xl font-semibold text-[var(--gw-ink)]">
                      Alle Gutscheine sind vergeben.
                    </h3>
                    <p className="mx-auto mt-3 max-w-xl text-[var(--gw-ink-soft)]">
                      Vielen Dank für das riesige Interesse! Die Gutscheinaktion ist damit
                      beendet – das Gewinnspiel läuft weiter.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div
                id="aktionsbedingungen"
                className="mt-8 scroll-mt-8 rounded-xl border gw-hairline bg-[var(--gw-surface)]/60 px-5 py-4"
              >
                <p className="text-xs font-semibold tracking-[0.2em] text-[var(--gw-gold)] uppercase">
                  Aktionsbedingungen
                </p>
                <ul className="mt-2 space-y-1.5 text-sm text-[var(--gw-ink-soft)]">
                  {GUTSCHEIN_BEDINGUNGEN.map((line) => (
                    <li key={line} className="flex gap-2">
                      <span
                        className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[var(--gw-gold)]"
                        aria-hidden="true"
                      />
                      {line}
                    </li>
        ...
        <section
          ...
          className={
            isGutschein
              ? "scroll-mt-8"
              : "scroll-mt-8 border-y gw-hairline bg-[var(--gw-bg-soft)]"
          }
        >
          <div className="mx-auto max-w-4xl px-5 py-16 sm:px-8 lg:py-20">
            <SectionHeading
        ...
        {/* 4 · Die Geschichte                                                */}
        {/* ---------------------------------------------------------------- */}
        <section className={isGutschein ? "relative border-y gw-hairline bg-[var(--gw-bg-soft)]" : "relative"}>
        ...
        <section
          ...
          className={isGutschein ? "" : "border-y gw-hairline bg-[var(--gw-bg-soft)]"}
        >
        ...
        {/* ---------------------------------------------------------------- */}
        {/* 6 · Gewinnspiel-Hinweis (sekundär)                                */}
        {/* ---------------------------------------------------------------- */}
      ...
      <footer className="border-t gw-hairline">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-5 py-10 text-sm text-[var(--gw-ink-mute)] sm:flex-row sm:px-8">
          ...
          >
            ...
            {isGutschein ? (
              <a href="#aktionsbedingungen" className="hover:text-[var(--gw-ink)]">
                Aktionsbedingungen
              </a>
            ) : null}
